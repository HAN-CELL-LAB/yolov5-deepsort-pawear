...
from yolov5.utils.torch_utils import time_sync as time_synchronized
...
def detect(opt):
    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if save_tsv and frame == 1:
                tsv_path = str(Path(out) / Path(p).stem) + '.tsv'
                # video_source, frame_idx, id, class_str, box_xc, box_yc, bbox_w, bbox_h
                with open(tsv_path, 'a') as f:
                    f.write(('%s ' * 8 + '\n') % ('video_source', 'frame_id', 'object_id', 'class_id',
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        if save_txt:
                            # to MOT format
                            # WTF top is horizontal here, so top is left and left is top
                            bbox_top = output[0]
                            bbox_left = output[1]
                        ...
                        if save_tsv:
                            ...
                            bbox_w = output[2] - output[0]
                            bbox_h = output[3] - output[1]
                            box_xc = box_x1 + bbox_w/2
                            box_yc = box_y1 + bbox_h/2

                            with open(tsv_path, 'a') as f:
                               f.write(('%s %g %g %s ' + '%g ' * 4 + '\n') % (Path(p).stem, frame, id, class_str,
                                                                              box_xc, box_yc, bbox_w, bbox_h))
    ...
    if save_txt or save_vid or save_tsv:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default='inference/working', help='output folder')  # output folder
